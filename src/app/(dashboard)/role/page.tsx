...
const transformApiData = (apiData: ApiRoleItem[]): RoleItem[] => {
  return apiData.map((item) => ({
    id: item.role_id,
    name: item.role_name ?? "",
    status: item.role_stat ? "Active" : "Inactive",
...
export default function UserTable() {
  ...
  const [sortField, setSortField] = useState<keyof RoleItem>("name");
  ...
  useEffect(() => {
    const fetchUserRoles = async () => {
      try {
        ...
        const cachedData = localStorage.getItem("role_cache");
        ...
        const response: ApiResponse = await api.get("/roles");
        ...
        const apiData: ApiRoleItem[] = Array.isArray(response.data)
          ? response.data
          : [response.data];

        const transformedData = transformApiData(apiData);

        localStorage.setItem("role_cache", JSON.stringify(transformedData));
