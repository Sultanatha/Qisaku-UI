...
interface ApiResponse {
  success: boolean;
  message: string;
  data: ApiUserRoleItem[]; // langsung array
}
...
const transformApiData = (apiData: ApiUserRoleItem[]): UserItem[] => {
  return apiData
    .filter((item) => item.user && item.role) // Pastikan user dan role tersedia
    .map((item) => ({
      id: item.id,
      name: item.user ?? "",
      email: item.email ?? "",
  ...
  useEffect(() => {
    const fetchUserRoles = async () => {
      try {
        ...
        const cachedData = localStorage.getItem("user_roles_cache");
        ...
        const response: ApiResponse = await api.get("/user-roles");
        ...
        const apiData: ApiUserRoleItem[] = Array.isArray(response.data)
          ? response.data
          : [response.data];
