...
interface ApiMenuItem {
  menus_id: number;
  menus_name: string; // nama user
  menus_path: string;
  parent: { menus_name: string } | null;
  menus_order: number;
  menus_stat: boolean;
  created_at: string;
}
...
interface ApiResponse {
  success: boolean;
  message: string;
  data: ApiMenuItem[]; // langsung array
}
...
const transformApiData = (apiData: ApiMenuItem[]): MenuItem[] => {
  // console.log("apiData:", apiData);
  return apiData.map((item) => ({
    id: item.menus_id,
    name: item.menus_name ?? "-",
    path: item.menus_path ?? "-",
    parent: item.parent?.menus_name || "-",
    order: item.menus_order,
    status: item.menus_stat ? "Active" : "Inactive",
...
export default function UserTable() {
  ...
  useEffect(() => {
    const fetchUser = async () => {
      try {
        ...
        const response: ApiResponse = await api.get("/menus");
        ...
        const apiData: ApiMenuItem[] = Array.isArray(response.data)
          ? response.data
          : [response.data];

        const transformedData = transformApiData(apiData);
        // console.log("Transformed Data:", transformedData);
        localStorage.setItem("menu_cache", JSON.stringify(transformedData));
  ...
  const { paginatedData, totalPages } = useTableData(data, {
    searchTerm,
    searchFields: ["name", "path", "parent", "order", "status"],
    sortField,
    sortDirection,
    currentPage,
    pageSize,
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {paginatedData.map((user) => (
                  ...
                  >
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span className="text-gray-600">{user.path}</span>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span className="text-gray-600">{user.parent}</span>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span className="text-gray-600">{user.order}</span>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span
